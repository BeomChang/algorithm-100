...
schedules = []
for i in range(n):
    schedules.append(list(input().split()))

# 년, 월, 일 순으로 정렬
# 날짜가 동일한 경우 스케줄 이름을 사전식 오름차순 정렬
sorted_schedules = sorted(schedules, key=lambda schedule: (int(schedule[1]), int(schedule[2]), int(schedule[3]), schedule[0]))
for i in range(len(sorted_schedules)):
    print(sorted_schedules[i][0])